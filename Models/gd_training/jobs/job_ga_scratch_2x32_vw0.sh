#!/bin/bash -l
#SBATCH --job-name=tc-ga-scratch-vw0
#SBATCH --account=nano
...
#SBATCH --requeue
#SBATCH --output=ga-scratch-vw0-%j.out

module load ml/pytorch

# GA-train a 2x32 thermo computer FROM SCRATCH (fresh GD warm start via
# run(seed=4), NOT the saved champion), 6000 generations, PURE BIAS fitness
# (var_weight = 0).  digital_net stays at its default 2x32 (no TC_WIDTH/DEPTH).
#
...
#   runs/ckpt_ga_finetune_thermo_seed4_g6000_vw0_M500_crn.pt
# and RESUMES from it automatically; if the job is ever pre-empted or over-runs,
# just resubmit the SAME job to continue:
#   sbatch jobs/job_ga_scratch_2x32_vw0.sh
# Each checkpoint also refreshes runs/run_ga_finetune_thermo_seed4_g6000_vw0_M500_crn.npz
# with the best-so-far model, so a killed run is never wasted.  The checkpoint
# is deleted on clean completion.
#
# Args: seed=4, generations=6000, var_weight=0, activation=thermo (no warm file).
...
python -u experiments/experiment_ga_finetune.py 4 6000 0 thermo
