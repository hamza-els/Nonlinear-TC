"""Thermo-activation teacher at three clock times, 5 seeds each.

Same structure as experiment_cos4_tf.py, but the target is the standard
cos(2 pi z) and the teacher uses the fitted thermodynamic-neuron activation

    sigma(x) = 0.5617 * (x^2/(x^2+13.53)) * cbrt(x) + tanh(x)

instead of tanh.  For each tf in {0.5, 1.0, 1.5} and seed in 0..4 we train a
fresh computer, record its RMSE at its own clock (M=256, K=250), and sweep phi
over observation times with parameters frozen.  The figure shows the three
clocks side by side, one translucent curve per seed, dotted line at the
trained tf.

Usage:
    python experiments/experiment_thermo_tf.py          # scan (+figure)
    python experiments/experiment_thermo_tf.py plot     # figure from saved npz
"""
...
NPZ = os.path.join(_GD_ROOT, "runs", "thermo_tf_results.npz")
...
def scan():
    ...
    t0 = time.time()
    print(f"config: activation=thermo TARGET_FREQ={TARGET_FREQ} seeds={SEEDS} "
          f"tfs={TF_LIST} sweep_pts={len(SWEEP)} K={K_TRAIN} M_eval={M_EVAL} "
    ...
    for si, seed in enumerate(SEEDS):
        ...
        for ti, tf in enumerate(TF_LIST):
            ...
            rmse[ti, si] = ev["rmse"]
            var[ti, si] = ev["var"]
            save_student(student, f"thermotf_tf{tf:g}_seed{seed}",
                         meta={"experiment": "thermo_tf", "activation":
                               "thermo", "tf": tf, "seed": seed,
...
def plot():
    ...
    fig.suptitle(r"thermo-activation teacher, target $\cos(2\pi z)$: $\phi$ vs "
                 rf"observation time, {phi.shape[1]} seeds per clock "
                 r"(dotted = trained $t_f$)", fontsize=13)
    fig.tight_layout(rect=(0, 0, 1, 0.94))
    out = os.path.join(GDIR, "fig_thermo_tf_grid.png")
