#!/bin/bash -l
#SBATCH --job-name=tc-ga-mnist-tf04-vw0
#SBATCH --account=nano
...
#SBATCH --time=4:00:00
#SBATCH --requeue
#SBATCH --output=ga-mnist-tf04-vw0-%j.out

module load ml/pytorch

# GA fine-tune of the PER-TF-REFIT MNIST thermodynamic computer at tf = 0.4,
# with var_weight = 0.
#   fitness = CE(<x_out(tf)>, label) + 0 * mean Var[x_out]
...
# Checkpoints every 25 gens to runs/ckpt_ga_mnist_tf0.4_vw0_seed0_g1000.pt
# and resumes on resubmission.
# Outputs: runs/ga_mnist_tf0.4_vw0_seed0_g1000.npz (champion params +
...
#
# Args: generations=1000, var_weight=0, tf=0.4, seed=0.
...
python -u experiment_ga_mnist.py 1000 0 0.4 0
